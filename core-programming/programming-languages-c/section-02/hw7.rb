...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(@x1,@y1,@x2,@y2)
      Point.new(@x1,@y1)
    elsif not(real_close(@x1,@x2)) and @x2 < @x1
      LineSegment.new(@x2,@y2,@x1,@y1)
    elsif real_close(@x1, @y1) and not(real_close(@y1,@y2)) and @y2 < @y1
      LineSegment.new(@x2,@y2,@x1,@y1)
    else
      self
    end
  ...
  def intersectWithSegmentAsLineResult seg
    # Took from peer review solution because i didn't get this one right
    if real_close(@x1,@x2)
